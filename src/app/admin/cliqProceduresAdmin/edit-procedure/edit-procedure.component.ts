...
export class EditProcedureComponent {
  ...
  loadData(){
    this.idMedico = this.state.Medico.id_medico;
  ...
  changeDoctor(){
    this.procedureForm.get('paciente')?.reset();
  ...
  editProcedure(){
    ...
    this.cliqProceduresService.editProcedure(object, this.idReserva).subscribe({
      complete: () => {
        ...
        this.router.navigateByUrl('admin/cliqProcedures/calendarProcedures', {replaceUrl: true});
      },
      error: (data) => {
